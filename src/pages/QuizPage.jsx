...
const QuizPage = () => {
	...
	const { quizLink, quiz, setQuiz, questionsAnswered, isCorrect, setIsCorrect, isIncorrect, setIsIncorrect } = useContext(QuizContext);
	...
	return (!questionsRandomized || questionsAnswered >= quizType[1]) ? (
		...
		<>
			<Navbar/>
			<div className="flex flex-col w-full my-10 items-center">
				<div className="w-[48rem] h-32">
					<h1 className="font-alte-bold text-2xl text-white text-center">#{questionsAnswered + 1}: {quiz[questionsAnswered].question}</h1>
				</div>

				<div className="grid md:grid-cols-2 grid-cols-1 mt-20 gap-6">
